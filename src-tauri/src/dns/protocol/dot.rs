...
pub async fn send_query(
    ...
    let tcp_stream = tokio::select! {
        ...
        _ = timeout => {
            return Err(anyhow::anyhow!("DoT connection timeout"));
        }
    };
    ...
    let tls_future = connector.connect(domain, tcp_stream);
    let timeout = tokio::time::sleep(tokio::time::Duration::from_millis(timeout_ms));

    let mut stream = tokio::select! {
        result = tls_future => result?,
        _ = timeout => {
            return Err(anyhow::anyhow!("DoT TLS handshake timeout"));
        }
    };
    ...
    tokio::select! {
        ...
        _ = timeout => {
            return Err(anyhow::anyhow!("DoT read timeout"));
        }
    }
    ...
    tokio::select! {
        ...
        _ = timeout => {
            Err(anyhow::anyhow!("DoT read timeout"))
        }
    }
}
